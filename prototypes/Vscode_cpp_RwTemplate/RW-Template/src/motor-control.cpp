...
// ============================================================================
// AprilTag POSITION RESET FUNCTIONS
// ============================================================================

/*
 * resetPositionWithAprilTags
 * Resets position using AprilTag detections from the vision sensor.
 * - tag_id: ID of the AprilTag to use for resetting position.
 * - field_half_size: Half the field dimension (distance from center to wall, in inches)
 * - tag_size: Physical size of the AprilTag (in inches)
 */
void resetPositionWithAprilTags(int tag_id, double field_half_size, double tag_size)
{
  ...
  {
    // Max detection range 24" so don't expect more than one tag in view at a time
    vex::aivision::object tag = aiCam.objects[0];
    ...
    if (tag.exists)
    {
      ...
      Pose2D robotPose = myTagMap.whereAmI(tag.id, tag.centerX, tag.centerY, tag.width, tag.height, gyroHeading);

      // Update global position based on tag detection
      if (robotPose.isValid())
      {
        x_pos = robotPose.x;
        y_pos = robotPose.y;
        correct_angle = robotPose.theta;
      }
    }
  }
}
